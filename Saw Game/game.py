...
def execute_take(id):
    ...
    for item in current_room['items']:
        ...
        elif item['id'] != id and item == current_room[len(current_room['items'])-1]:
...
def main(global_timer):
    ...
    # Main game loop
    while True: 
        print("Time remaining:", global_timer[0])
        # Display game status (room description, inventory etc.)
        print(current_room['map'])
        #print_room(current_room)
        print_inventory_items(inventory)
        scrollTxt(current_room['description'])
...
if __name__ == "__main__":
    ...
    timer_thread = threading.Thread(target=countdown_timer, args=(20, global_timer))
